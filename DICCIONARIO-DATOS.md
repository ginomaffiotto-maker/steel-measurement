...
| `autor` | text | Nombre, texto libre — se sigue mostrando así en la UI. |
| `autor_id` | uuid → `profiles` on delete set null | 2026-09-12 — antes el permiso de borrado comparaba por `autor` (nombre, sin FK real); ahora hay una columna real, completada sola por trigger en cada INSERT nuevo, con backfill best-effort de lo viejo (matchea `autor` contra `profiles.nombre` del mismo tenant, solo si matchea a exactamente una persona). El DELETE (RLS) queda restringido a `autor_id = auth.uid()` o admin/supervisor — antes cualquiera del tenant podía borrar un comentario ajeno pegándole directo a la API, la regla solo vivía en la UI. |
| `texto` | text not null | |
...
| `mensaje_cliente` | text | 2026-09-12 — el mail/WhatsApp original del cliente, pegado tal cual (`Solicitudes.jsx`). Existía en el formulario desde antes pero nunca se sincronizaba; se agregó para que "Mis solicitudes asignadas" (Steel Costos, que lee esta tabla directo) pueda mostrarlo. |
...
| `estado_crm` | text, nullable | 2026-09-04 — espejo inverso de `estado_sm` (Steel CRM local), escrito desde Steel CRM cuando cambia el `estado_nativo` de un presupuesto ya vinculado. Hasta el 2026-09-11 era solo informativo; desde el 2026-09-12 la RPC `actualizar_estado_crm_en_costos()` también actualiza `estado` (arriba) al Aceptar/No aprobar/Reabrir en Steel CRM — ver `ENTIDADES-COMPARTIDAS.md` §6. |
| `solicitud_id` | uuid → `solicitudes` on delete set null | 2026-09-12 — directo, no solo heredado vía `codigo_calculo`/`computo_id`: permite crear un Presupuesto de Steel Costos directo desde una Solicitud de Steel CRM, saltando Cómputo. |
...
### `anidados`
`nombre`, `fecha`, `cliente_id → clientes`, `obra`, `empresa`, `categoria`, `tipo_trabajo`, `vendedor → profiles`. ✅ soft-delete (mismo momento y motivo que `computos`). `obra_id → obras` on delete set null, agregada 2026-08-29 — convive con `obra` (texto), mismo criterio que `presupuestos_sm`. `empresa_id → empresas` on delete set null, mismo commit — `empresa` (texto) ya existía en la tabla pero nunca se sincronizaba (bug real, cerrado el mismo día: faltaba en el allowlist de columnas de `storage.js`). `link_archivos` (text, 2026-09-06). `computo_id → computos` on delete set null (2026-09-12) — "Importar desde cómputo" siempre copió materiales/categoría/vendedor desde el cómputo de origen, pero nunca había guardado el vínculo en sí; cierra la trazabilidad Solicitud→Cómputo→Anidado→Presupuesto (mismo criterio que `computos.solicitud_id`, 2026-09-05). `solicitud_id → solicitudes` on delete set null (mismo commit) — directo, para poder crear un Anidado sin pasar por Cómputo.
...
Misma forma que los comentarios de Steel CRM (`computo_id`/`anidado_id`/`presupuesto_id` → tabla padre cascade, `autor`, `texto`, `fecha`, `hora`). Sin columnas de soft-delete propias en la migración original de Steel Costos — a diferencia de las de Steel CRM, que sí las ganaron después (`soft_delete_comentarios`, ver §0). Confirmar contra el código si esto cambió antes de asumir. `autor_id → profiles` on delete set null (2026-09-12) — mismo patrón que las 3 tablas de comentarios de Steel CRM (trigger + backfill best-effort), DELETE (RLS) restringido a autor propio o admin/supervisor.
