...
//
// La ficha (ver/crear/editar) vive en FichaSolicitud.jsx — 2026-09-14, a
// pedido de Gino: "que funcione como la de CRM" (buscadores con alta
// rápida de Cliente/Obra/Empresa, Categoría obligatoria, candado de dueño
// visible), sin necesidad de verse igual ni tener la misma cantidad de
// datos. Antes esto era una ficha de solo lectura + un modal de alta
// aparte con campos de texto libre — ahora es un solo componente dual.

export default function SolicitudesAsignadas({ usuario, irATab }) {
  const [solicitudes, setSolicitudes] = useState([]);
  const [cargando, setCargando] = useState(true);
  const [error, setError] = useState("");
  // null = cerrada, "nueva" = alta, objeto = ver/editar una existente.
  ...
  return (
    <div>
      {ficha && (
        <FichaSolicitudModal s={ficha === "nueva" ? null : ficha} usuario={usuario} onClose={() => setFicha(null)}
          onCreated={s => setSolicitudes(prev => [s, ...prev])}
          onSaved={s => setSolicitudes(prev => (
            // Un cierre (ganada/perdida/no cotizado) deja de matchear el
            // filtro de la query de arriba — se saca de esta lista en vez
            // de mostrar una fila que ya no correspondería a un reload.
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4, flexWrap: "wrap", gap: 8 }}>
        ...
        <button onClick={() => setFicha("nueva")} style={{ ...BTN("primary"), fontSize: 12 }}>+ Nueva solicitud</button>
      </div>
      ...
      {!cargando && solicitudes.length > 0 && (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            ...
            <tbody>
              {lista.map(s => (
                <tr key={s.id} onClick={() => setFicha(s)} style={{ cursor: "pointer" }}
